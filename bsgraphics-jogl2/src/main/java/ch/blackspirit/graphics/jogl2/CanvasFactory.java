...
public final class CanvasFactory implements ch.blackspirit.graphics.CanvasFactory {
	...
	public RealtimeCanvas createRealtimeCanvasFullscreen(DisplayMode displayMode) {
		return new ch.blackspirit.graphics.jogl2.RealtimeCanvas(displayMode, properties);
	}
	public RealtimeCanvas createRealtimeCanvasFullscreen() {
		return new ch.blackspirit.graphics.jogl2.RealtimeCanvas(null, properties);
	}
	public RealtimeCanvas createRealtimeCanvasWindow(int width, int height) {
		return new ch.blackspirit.graphics.jogl2.RealtimeCanvas(width, height, properties);
	}
	...
	public Set<DisplayMode> getDisplayModes() {
		...
		for(java.awt.DisplayMode mode :dev.getDisplayModes()) {
			modes.add(new ch.blackspirit.graphics.jogl2.DisplayMode(mode.getWidth(), mode.getHeight(), mode.getBitDepth(), mode.getRefreshRate()));
		}
		return modes;
	}
	/* (non-Javadoc)
